...
Illustration of the B-spline :math:`\beta,` the cardinal B-spline :math:`\eta,` the dual B-spline :math:`\mathring{\beta},` and the orthonormal B-spline :math:`\phi.`
...
We are first going to give the intuition of what a basis is, by making a detour with the representation of numbers. Then, we introduce uniform splines as weighted sums of integer-shifted synthesis functions. We go into greater depth in our general discussion of synthesis functions, and then focus on the specific case of the B-spline, of which we give just a few of their numerous properties. We end with the illustration of three other kinds of synthesis functions.
...
Digits are the construction blocks of numbers. They are associated to a *base*; in everyday life, the base is ten. In a computer, the binary base is two. Geeks would write octal numbers in base eight or hexadecimal numbers in base sixteen. Assuming the base is written as a base-ten subscript, one would notate
...
There, the indexed weights :math:`\left(c[k]\right)_{k\in{\mathbb{Z}}}` play the same role as the digits :math:`\left(d[k]\right)_{k\in{\mathbb{Z}}}` of numbers, while the synthesis function :math:`\varphi` plays the same role as the base :math:`b` of numbers. However, whereas the base was raised to the integer power :math:`k,` the synthesis function now is shifted by the integer amount :math:`k.` In other words, the coefficients are to the spline what the digits are to the string representation of a number, and the synthesis function is to the spline what the (decimal or binary) base is to the string representation of a number.
...
Given the number :math:`x`, many a base :math:`b` can be adopted to represent its value, with adjusted digits :math:`d`. Likewise, given a spline :math:`f`, many a synthesis function :math:`\varphi` can be adopted to represent the spline, with adjusted weights :math:`c`. For :math:`f` to be truly called a spline, however, the synthesis function itself must adhere to some specific constraints. As we consider only *polynomial* uniform splines of nonnegative integer degree :math:`n\in{\mathbb{N}}`, the synthesis function :math:`\varphi` must
...
In the sequel, the B-spline synthesis function provides the reference to which all other synthesis functions will be compared. We set
...
There exists a synthesis function :math:`\eta^{n}` that happens to be interpolating. It is called a cardinal B-spline. Some of its mathematical properties can be considered less favorable than those of B-splines; most notably, contrarily to B-splines, the support of cardinal B-splines is infinite, which makes it impractical. Yet, its interpolating property, according to which :math:`\eta^{n}(0)=1` and :math:`\eta^{n}(k)=0` for :math:`k\in{\mathbb{Z}}\setminus\{0\},` greatly facilitates the determination of the coefficients of the representation. Indeed, it is enough to let :math:`c_{2}=y` to automatically get that
...
The function :math:`f:{\mathbb{R}}\rightarrow{\mathbb{R}},\;x\mapsto\sum_{k\in{\mathbb{R}}}\,c[k]\,\varphi(x-k)` is a spline only for certain synthesis functions :math:`\varphi.` A famous case where :math:`f` fails to be a spline arises if one chooses :math:`\sin(\pi\,x)/\left(\pi\,x\right)={\mathrm{sinc}}\,x` to play the role of the synthesis function, which is associated to the celebrated Whittaker–Shannon interpolation formula and has strong theoretical properties.
...
Let the synthesis function :math:`\mathring{\beta}^{n_{2},n_{1}}` of dual degree :math:`n_{2}\in{\mathbb{N}}` and primal degree :math:`n_{1}\in{\mathbb{N}}` be called a dual B-spline. It is yet another synthesis function that is sometimes used to provide an alternate (equivalent) representation of the spline as
...
Let the orthonormal B-spline :math:`\phi^{n}` of degree :math:`n` be a final synthesis function of interest that is sometimes used to provide an alternate (equivalent) representation of the spline as
